...
- [nowork-studio/toprank](https://github.com/nowork-studio/toprank) 🎖️ ☁️ - **NotFair** Google Ads MCP server. Connect Claude and AI agents to a Google Ads account: diagnose campaign performance (CPA, ROAS, search-term waste, quality scores), recommend optimizations (bids, budgets, negatives, ad copy), and execute approved changes via the Google Ads API with a built-in human-approval gate. Streamable-HTTP at `notfair.co/api/mcp/google_ads`. Listed in the official MCP registry as `io.github.nowork-studio/notfair`.
